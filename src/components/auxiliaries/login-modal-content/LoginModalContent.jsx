        ...
        if (stage == 0 && props.content == "Log In") {
            ...
            setContentsToDisplay(
                <>
                    ...
                    <br />
                    {error ? <p className="toShowError">{error}</p> : <p className="gen-mess">By signing up, you consent to our use of your above entered information, and that you are older than or 13 years old.</p>}
                    <button onClick={async click => {
                        const emailTrim = email.trim();
                        ...
                        setStage(1);
                        try {
                            if (userNameTrim == "") { throw { code: "auth/null-username" }; }
                            else if (userNamesMatch.docs.length > 0) { throw { code: "auth/username-taken" }; }
                            else if (userNameTrim.contains(" ") || userNameTrim.contains("/")) { throw { code: "auth/invalid-username" } }
                            else if (emailTrim == "") { throw { code: "auth/invalid-email" } }
                            else if (password != confirmPassword) { throw { code: "auth/passwords-dont-match" }; }
                            ...
                            switch (error.code) {
                                ...
                                case "auth/too-many-requests": setError("We've received too many requests from your device, please try again later. 🤖🧑‍💻"); break;
                                case "auth/invalid-username": setError("Username can't contain any spaces or slashes.")
                                default: setError("There was an error. Please try again later or report the error via email."); break;
                            }
                            setStage(0);
                        }
